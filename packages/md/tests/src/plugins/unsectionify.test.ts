...
describe('unsectionify', () => {
	it('should flatten sections back to flat markdown', async () => {
...
Final content`
		...
		const tree = await processor.run(parsed) as Parent
...
Content 2`
	...
	it('should be roundtrip reversible with sectionify', async () => {
...
Content B`
		...
		const resultParsed = processor2.parse(markdown)
		const resultTree = await processor2.run(resultParsed) as Parent
	...
	it('should preserve content order when flattening', async () => {
		const markdown = `# H1
P1

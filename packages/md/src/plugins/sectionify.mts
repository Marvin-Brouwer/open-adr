	...
	transform(context) {
		...
		while (index < root.children.length) {
			...
			if (isHeading(node)) {
				...
				const { section, consumed } = buildSection(root.children, index)
...
interface BuildSectionResult {
	section: SectionNode
	consumed: number
}

function buildSection(children: Node[], startIndex: number): BuildSectionResult {
	...
	while (index < children.length) {
		...
		if (isHeading(node) && node.depth > headingDepth) {
			...
			const { section: nestedSection, consumed } = buildSection(children, index)

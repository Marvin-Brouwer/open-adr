...
export default function remarkHoverSchema() {
  return (tree, file) => {
    ...
    if (!schemaRef) return
    if (allowedSchemas.length > 0 && !allowedSchemas.includes(path.basename(schemaRef))) return
    ...
    if (Array.isArray(schema.requiredHeadings)) {
      schema.requiredHeadings.forEach(h => { if (h.text && h.description) headingDescriptions[h.text] = h.description })
    }
